...
# Quantization levels for INT4/INT2: (n_levels, max_val)
_QUANT_LEVELS = {
    Precision.INT4: (15, 15),
    Precision.INT2: (3, 3),
}
...
class AdaptiveQuantizer:
    ...
    def _quantize_single(
        ...
        if prec == Precision.FP16:
            ...
            qk, mk = AdaptiveQuantizer._quantize_per_axis(k, prec, reduce_dims=(0, 1, 2))
            qv, mv = AdaptiveQuantizer._quantize_per_axis(v, prec, reduce_dims=(0, 1, 3))
            return qk, qv, {
    ...
    def _quantize(tensor: torch.Tensor, precision: Precision) -> Tuple[torch.Tensor, dict]:
        ...
        elif precision in (Precision.INT4, Precision.INT2):
            n_levels, max_val = _QUANT_LEVELS[precision]
            mn, mx = f.min(), f.max()
    ...
    @staticmethod
    def _quantize_per_axis(tensor: torch.Tensor, precision: Precision,
                           reduce_dims: tuple) -> Tuple[torch.Tensor, dict]:
        """Per-axis quantization with independent scale/zero per slice.

        For Keys: reduce_dims=(0,1,2) → per-channel (head_dim axis).
        For Values: reduce_dims=(0,1,3) → per-token (seq_len axis).
        """
        f = tensor.float()
        n_levels, max_val = _QUANT_LEVELS[precision]
        mn = f.amin(dim=reduce_dims, keepdim=True)
        mx = f.amax(dim=reduce_dims, keepdim=True)
        scale = (mx - mn) / n_levels
